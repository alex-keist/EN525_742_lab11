...
#define RADIO_CLK_HZ 125000000.0
...
void print_benchmark(volatile unsigned int *periph_base)
{
	...
	unsigned int clocks_elapsed = stop_time - start_time;
	unsigned int bytes_transferred = 2048 * sizeof(unsigned int);
	double time_spent = (double)clocks_elapsed / RADIO_CLK_HZ;
	double throughput = (bytes_transferred / time_spent) / 1.0e6;
    printf("You transferred %u bytes of data in %lf seconds\n",bytes_transferred,time_spent);
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 10 Alex Keist - Custom Peripheral Demonstration\n\r");
